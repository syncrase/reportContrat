

\textbf{text gras}\hfill \\ \indent text indenté

%cf wikibook font size
\Large{}\normalsize
\LARGE{}\normalsize
\huge
\Huge


\footnote{Source : \url{}}

%Ne pas mettre de caractères accentués dans les titres ci-dessous
\chapter{chapX}\label{chapter:chapX}
\section{section}
\subsection{sous-section}
\subsubsection{sous-sous-section}


une ligne\\ une ligne à la ligne

\c{c} : ç
\'{e} : é
\`{e} : è
\`{a} : à
\^{a} : â
\^{e} : ê
\"{e} : ë
\oe{}
\{
\%
\_
\textbackslash{}


\textquote{derrière}


1\up{er}

%Juste une liste
\begin{itemize}
	\item un élément de liste\index{ref pour l'index}
\end{itemize}
%
\begin{description}
	\item[un titre en gras] \hfill \\
	une description indentée

\end{description}



%%IMAGES
\ref{figure:} page \pageref{figure:}

\begin{figure}[!h]% h || ht || .... cf 
  \centering
      \includegraphics[width=\textwidth+1cm,height=\textheight-1.5cm]{images/.jpg}
  \caption{}
	\label{figure:}
\end{figure}


%% PDF
\includepdf[pages=-,scale=0.9,pagecommand={}\label{pdf:org}]{images/AutomationResults.pdf}

%%% adapt geometry
%% 0 partout et sa "supprime" header footer etc qui se retrouvent à l'extérieur
\newgeometry{left=1cm,right=1cm,top=0cm,bottom=2cm}
\restoregeometry

%Insérer du code
%Attention à la config nécessaire !
\lstinputlisting[language=java]{scripts/basicStaticTest.java}



%plus simple mais verbeux
\begin{lstlisting}
MonoDocTestCase(MonoDocTestCaseConfigInfo tccInfo, String sDocumentName, String sCategoryType, String sCategory, Boolean useAuroraCtx);
\end{lstlisting}



% Pour les longs mots, ceux-ci se retrouvent coupés si c'est déconné
\begin{sloppypar}
\end{sloppypar}


%%%% Pour avoir un lien internet actif
\url{https://url}


%% TABLE
\begin{tabular}{|l|c|r|}
  \hline
  colonne 1 & colonne 2 & colonne 3 \\
  \hline
  1.1 & 1.2 & 1.3 \\
  2.1 & 2.2 & 2.3 \\
  \hline
\end{tabular}


\begin{table}[h!]
	\begin{center}
     \begin{tabular}{ c  p{5cm}  p{5cm}  }
			 \toprule
				my.Lboro & Advantages & Disadvantages \\ 
			\cmidrule(r){1-1}\cmidrule(lr){2-2}\cmidrule(l){3-3}
			 \raisebox{-\totalheight}{\includegraphics[width=0.3\textwidth, height=50mm]{images/sap-executive-mcdermott.jpg}}
				& 
				\begin{itemize}[topsep=0pt]
				\item Accessibility
				\item Up to date information
				\item Fulfil students needs and wants \ldots
				\end{itemize}
				& 
				\begin{itemize}[topsep=0pt]
				\item Accessibility
				\item Up to date information
				\item Fulfil students needs and wants \ldots
				\end{itemize}
				\\ 
				
				\bottomrule
				\end{tabular}
		\caption{my.Lboro Analysis}
		\label{tbl:myLboro}
	\end{center}
\end{table}





\usepackage{textcomp}%permet l'utilisation du tild  \texttildelow

\hrulefill%trace un trait horizontal
\dotfill%Rempli la ligne avec des points

\parindent=-50pt %configuration l'indentation


% Raccourcis
%Ctrl + Q 			commenter
%Ctrl + W			Décommenter